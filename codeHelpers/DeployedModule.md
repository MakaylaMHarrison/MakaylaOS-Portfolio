...
// CustomLaserDivider - Sized to 55%, animates on view, and features an active theme background scan
...
const DeployedModule = () => {
  // activePipeline tracks specific steps now (e.g., 'm01_step1')
    ...
    <div className="relative w-full px-6 pb-8 pt-0 mt-0 font-mono text-slate-300 flex flex-col justify-between">
      <div className="relative z-10 w-full flex-grow flex flex-col mt-0 pt-0">
        ...
        <div className="mb-6 pb-4 text-center max-w-3xl mx-auto w-full mt-0 pt-0 flex flex-col items-center">
          ...
          {/* Main Module Section Divider - Sized to 45%, triggers on scroll, and infinitely loops a background scanning motion */}
                  ...
                  <div>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="mb-3 text-xs text-slate-400 leading-relaxed border-l-2 border-cyan-500/30 pl-2.5 py-0.5">
                        <span className="text-white font-bold tracking-wider">SYSTEM_BRIEF:</span> A lightweight decision-orientation engine that classifies a user's work situation into system phases, role types, and pressure patterns, rendering a personalized system map using structured narrative templates.
                      </div>
                    ...
                    {/* Step Matrix Blocks - Scoped Hover Logic */}
                    <div className="space-y-2.5 relative flex-grow mb-4">
                      ...
                      >
                        ...
                        <ul className={`space-y-1 text-xs list-none pl-0 mb-0 transition-colors ${activePipeline === 'm01_s1' ? 'text-slate-200' : 'text-slate-400'}`}>
                          <li>↳ <span className={activePipeline === 'm01_s1' ? 'text-cyan-300' : 'text-cyan-500/90'}>role_taxonomy</span>: active</li>
                          <li>↳ <span className={activePipeline === 'm01_s1' ? 'text-cyan-300' : 'text-cyan-500/90'}>pressure_flags</span>: checked</li>
                        </ul>
                      </div>
...
function LocalMobileCircuitLine({ colorHex }) {
  return (
    <div className="w-full h-8 flex justify-center items-center bg-transparent overflow-visible my-1">
      <svg viewBox="0 0 20 32" className="h-full w-4 overflow-visible">
        <Motion.path
          d="M 10,0 L 10,32"
          fill="none"
          stroke={colorHex}
...
export default function DeployedModule() {
  const [activePipeline, setActivePipeline] = useState(null);
  const [persistentTerminalId, setPersistentTerminalId] = useState(null);
  const [expandedBriefs, setExpandedBriefs] = useState({ m01: false, m02: false, m03: false });
  ...
  return (
    <div className="relative w-full px-4 sm:px-6 pb-8 pt-0 mt-0 font-mono text-slate-300 flex flex-col justify-between select-none">
      <div className="relative z-10 w-full flex-grow flex flex-col mt-0 pt-0">
        ...
        {/* ==================== HUD INTERFACE GRID ==================== */}
        <div className="flex flex-col lg:grid lg:grid-cols-3 gap-8 lg:gap-6 items-start">
          
          {/* ==================== MODULE 01 (CYAN) ==================== */}
          {(() => {
            const isCardActive = activePipeline?.startsWith('m01') || (!activePipeline && persistentTerminalId?.startsWith('m01'));
            const theme = moduleThemes.m01;
            return (
              <div className="w-full flex flex-col">
                <div className="relative w-full rounded-lg bg-[#070d19]/20 backdrop-blur-3xl p-1 min-h-[360px] lg:min-h-[380px] flex flex-col z-10">
                  <HudCorners isActive={isCardActive} activeColor={theme.baseHex} />
                  ...
                  >
                    <div>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="mb-3 text-xs text-slate-400 leading-relaxed border-l-2 border-cyan-500/30 pl-2.5 py-0.5">
                          <span className="text-white font-bold tracking-wider">SYSTEM_BRIEF:</span>{' '}
                          <span className={expandedBriefs.m01 ? '' : 'line-clamp-2 lg:line-clamp-none'}>
                            A lightweight system-modeling tool that models organizational behavior under stress.
                          </span>
                          <button 
                            onClick={() => toggleBrief('m01')} 
                            className="text-cyan-400 text-[10px] font-bold block lg:hidden mt-1 hover:underline tracking-widest"
                          >
                            {expandedBriefs.m01 ? '[ - SHOW_LESS ]' : '[ + EXTEND_BRIEF ]'}
                          </button>
                        </div>
                      ...
                      <div className="space-y-2.5 relative flex-grow mb-4">
                        <div 
                          className={`p-2.5 rounded-md border cursor-pointer transition-all duration-200
                            ${activePipeline === 'm01_s1' || (persistentTerminalId === 'm01_s1' && !activePipeline) ? `${theme.borderActiveStep} ${theme.bgActiveStep}` : 'border-slate-800/80 bg-[#0a1424]'}`}
                          onMouseEnter={() => { setActivePipeline('m01_s1'); setPersistentTerminalId('m01_s1'); }}
                          onMouseLeave={() => setActivePipeline(null)}
                          onClick={() => setPersistentTerminalId('m01_s1')}
                        >
                          <div className={`${activePipeline === 'm01_s1' || persistentTerminalId === 'm01_s1' ? theme.textAccent : 'text-white'} font-bold mb-1 flex items-center justify-between text-xs transition-colors`}>
                            <span>01.   INPUT_LAYER</span>
                          </div>
                        </div>
                      </div>
                ...
                {/* ATTACHED LOCAL MOBILE CONSOLE DRAWER */}
                <AnimatePresence>
                  {persistentTerminalId?.startsWith('m01') && (
                    <Motion.div 
                    ...
                    >
                      <LocalMobileCircuitLine colorHex={theme.baseHex} />
                      {/* FIXED: Removed outer wrapper completely, mounting Terminal bare */}
                      <Terminal targetModuleId={persistentTerminalId} />
                    </Motion.div>
                  )}
                </AnimatePresence>
          ...
          {/* ==================== MODULE 02 (AMBER) ==================== */}
          {(() => {
            const isCardActive = activePipeline?.startsWith('m02') || (!activePipeline && persistentTerminalId?.startsWith('m02'));
            const theme = moduleThemes.m02;
            return (
              <div className="w-full flex flex-col">
                <div className="relative w-full rounded-lg bg-[#070d19]/20 backdrop-blur-3xl p-1 min-h-[360px] lg:min-h-[380px] flex flex-col z-10">
                ...
                {/* ATTACHED LOCAL MOBILE CONSOLE DRAWER */}
                <AnimatePresence>
                  {persistentTerminalId?.startsWith('m02') && (
                    <Motion.div 
                      initial={{ height: 0, opacity: 0 }}
                      animate={{ height: "auto", opacity: 1 }}
                      exit={{ height: 0, opacity: 0 }}
          ...
          {/* ==================== MODULE 03 (PURPLE) ==================== */}
          {(() => {
            const isCardActive = activePipeline?.startsWith('m03') || (!activePipeline && persistentTerminalId?.startsWith('m03'));
            const theme = moduleThemes.m03;
            return (
              <div className="w-full flex flex-col">
                <div className="relative w-full rounded-lg bg-[#070d19]/20 backdrop-blur-3xl p-1 min-h-[360px] lg:min-h-[380px] flex flex-col z-10">
                ...
                {/* ATTACHED LOCAL MOBILE CONSOLE DRAWER */}
                <AnimatePresence>
                  {persistentTerminalId?.startsWith('m03') && (
                    <Motion.div 
                      initial={{ height: 0, opacity: 0 }}
                      animate={{ height: "auto", opacity: 1 }}
                      exit={{ height: 0, opacity: 0 }}
